...
app.controller('oneCtrl',["$scope","$http",'$sce','$rootScope','$interval',function($scope,$http,$sce,$rootScope,$interval){
    ...
    $scope.show = function() {
    	...
    	console.log($rootScope.close,0);
 	};
 	$scope.hided = function(){
 		$rootScope.close = true;
 		console.log($rootScope.close,1);
 	}

 	$scope.change = function(url,id,img,title,content,e){
 		e = e || window.event;
 		e.stopPropagation();
 		$rootScope.close = true;
		$rootScope.URL = url;
...
app.controller('searchCtrl',['$scope',"$http","$rootScope",function($scope,$http,$rootScope){
	...
	$http.get('http://route.showapi.com/213-1?showapi_appid=26496&showapi_sign=7409e7919e674415a45e5b479bec39e6&keyword=海阔天空&page=1').success(function(data){
		console.log(data)
	})
}])